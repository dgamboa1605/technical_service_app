...
const STATUS_BADGE: Record<WorkOrderStatus, string> = {
  ...
  por_confirmar: "bg-gray-100 text-gray-800",
...
// Flujo: RECIBIDO -> ASIGNADO -> POR_CONFIRMAR -> CONFIRMADO -> EN_REPARACION -> COMPLETADO -> ENTREGADO
...
export default function WorkOrderDetailPage() {
  ...
  const grandTotal = useMemo(() => {
    if (!detail) return 0;
    return partsTotal + (detail.labor_cost || 0);
  }, [detail, partsTotal]);
  ...
  const loadDetail = async () => {
    ...
    try {
      ...
      setDetail(data);
      setReport(data.technical_report || "");
  ...
  const handleAdvanceStatus = async (to: WorkOrderStatus) => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.updateStatus(detail.id, { status: to, note: note || undefined });
      setNote("");
      await loadDetail();
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateReport = async () => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.updateTechnicalReport(detail.id, report);
      await loadDetail();
  ...
  const handleAddHistory = async () => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.addHistory(detail.id, { note });
      setNote("");
      await loadDetail();
  ...
  const handleAddPart = async () => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.addPart(detail.id, {
  ...
  const handleAssignTechnician = async () => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.assignTechnician(detail.id, parseInt(selectedTechnician));
  ...
  const handleUpdateLaborCost = async () => {
    ...
    setSaving(true);
    try {
      await workOrdersApi.updateLaborCost(detail.id, laborCost);
      await loadDetail();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 print:hidden">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Recepción: {formatDate(detail.received_date)}</p>
            </div>
            <div className="text-right space-y-2">
              <div className="flex items-center gap-2 justify-end">
                <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${STATUS_BADGE[detail.status]}`}>
              ...
              <div className="flex flex-wrap gap-2 justify-end">
                {!detail.technician && detail.status === 'recibido' && (
                  <button
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">Tipo: {detail.service_type.toUpperCase()}</p>
            </div>
          </div>
        </section>
            ...
            <div className="space-y-2">
              <div className="bg-slate-50 dark:bg-slate-900/20 p-2 rounded-lg">
                ...
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  {detail.product ? `${detail.product.item_type} ${detail.product.brand}` : '-'}
                </p>
              ...
              <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                <span>Serie: <span className="font-mono font-medium">{detail.product?.serial_number || '-'}</span></span>
              </div>
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                {detail.product?.warranty && detail.product?.guaranteeing_brand && (
                  <p className="text-xs text-gray-600 dark:text-gray-400">Marca: {detail.product.guaranteeing_brand}</p>
                )}
                {detail.product?.purchase_date && (
                  <p className="text-xs text-gray-600 dark:text-gray-400">Compra: {formatDate(detail.product.purchase_date)}</p>
                )}
              </div>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="space-y-2">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg">
                <p className="text-sm font-bold text-gray-900 dark:text-white">{detail.client?.name}</p>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                </svg>
                <span>{detail.client?.document_number || '-'}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid md:grid-cols-2 gap-3 print:hidden">
          <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 space-y-3">
            ...
            <div className="border-l-4 border-orange-400 bg-orange-50 dark:bg-orange-900/20 p-2 rounded-r">
              ...
              <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                {detail.customer_instructions || 'Sin indicaciones'}
              </p>
            </div>
            ...
            <div className="border-l-4 border-slate-400 bg-slate-50 dark:bg-slate-900/20 p-2 rounded-r">
              ...
              <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                {detail.item_condition || 'No especificado'}
              </p>
            </div>
            ...
            <div className="border-l-4 border-green-400 bg-green-50 dark:bg-green-900/20 p-2 rounded-r">
              ...
              <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                {detail.delivered_accessories || 'Ninguno'}
              </p>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 space-y-3">
            ...
            <div className="border-2 border-blue-200 dark:border-blue-800 rounded-lg p-3 bg-blue-50/50 dark:bg-blue-900/10">
              <div className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap min-h-[120px]">
                {detail.technical_report || (
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm font-medium transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
              {detail.technical_report ? 'Editar informe técnico' : 'Agregar informe técnico'}
            </button>
          </section>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-3 print:hidden">
          <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 flex flex-col">
            ...
            <div className="space-y-2 overflow-y-auto mb-3 flex-1">
              {detail.history.length === 0 && <p className="text-sm text-gray-500 dark:text-gray-400">Sin registros</p>}
              {detail.history.map((h) => (
                <div key={h.id} className="rounded border border-gray-100 p-2 text-sm dark:border-gray-700">
                  <div className="flex justify-between items-start mb-1">
                    <p className="text-xs text-gray-500 dark:text-gray-400">{formatDate(h.created_at)}</p>
                    {h.user && (
                  ...
                  <p className="font-semibold text-gray-900 dark:text-white">
                    {h.status_to ? STATUS_LABEL[h.status_to] : "Nota"}
                  </p>
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 flex flex-col">
            <h2 className="text-lg font-semibold mb-4 pb-3 border-b border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white">Costos y Repuestos</h2>
            <div className="flex-1 flex flex-col gap-3">
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="space-y-2 flex-1 overflow-y-auto">
                  {detail.parts.length === 0 && <p className="text-sm text-gray-500 dark:text-gray-400">Sin repuestos</p>}
                  {detail.parts.map((p) => (
                    <div key={p.id} className="flex justify-between text-sm border-b border-gray-100 pb-2 dark:border-gray-700">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-xs text-gray-500 dark:text-gray-400">{p.qty} x ${p.unit_price.toFixed(2)}</p>
                          {p.user && (
            ...
            <button
              ...
              className="w-full px-4 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700 mt-3"
            >
              Agregar repuesto
            </button>
          </section>
