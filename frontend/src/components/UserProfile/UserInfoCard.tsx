...
export default function UserInfoCard() {
  ...
  const handleSave = async () => {
    ...
    const hasChanges = formData.username.trim() !== user.username || formData.email.trim() !== user.email;
    if (!hasChanges) {
      closeModal();
      return;
    }
    ...
    try {
      ...
      // Solo enviar si hay actualizaciones
      ...
      const errorMessage = error?.message || error?.response?.data?.detail || "Error al actualizar el perfil. Por favor, intenta de nuevo.";
      alert(errorMessage);
    } finally {
      setIsSaving(false);
    }
